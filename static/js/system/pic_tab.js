...
function loadPicTab() {
	...
	jq.fn.banqh = function(can) {
		...
		jq(can.pnum).find('ul').width(picminnum * picminw).height(
				picminnum * picminh);
		...
		function show(tpqhnum) {
			...
			// jq(can.pic).find('ul
			// li').eq(tpqhnum).fadeIn(can.delayTime).siblings('li').fadeOut(can.delayTime);//淡入淡出
			jq(can.pnum).find('li').eq(tpqhnum).addClass("on").siblings(this)
					.removeClass("on");
		}
		;
		// 弹出框图片切换过程
		function popshow(popnum) {
			...
			// jq(can.pop_pic).find('ul
			// li').eq(tpqhnum).fadeIn(can.pop_delayTime).siblings('li').fadeOut(can.pop_delayTime);//淡入淡出
		}
		;
