	...
	$.fn.upload = function(options,param){
		...
		return this.each(function(){
			...
			var defaults = {
					...
					rootFolderName	 : "",   						//附件所属类型  1 高支模 2 深基坑 3 塔吊 4 扬尘噪声
					showUploadButton : true ,						//是否单独显示上传按钮
					type : "" ,										//文件所属类型
					relatedId		 : ""							//附件所属记录/表单的id
			};
			//初始化前清空参数值
			FILE.fileInput = null;
			...
			this.createHtml = function(){
				var multiple = false;  // 设置多选的参数
				para.multiple ? multiple = true :  "";
				var html= '';
				if(para.dragDrop){
					// 创建带有拖动的html
					html += '<form id="uploadForm" action="'+para.url+'" method="post" enctype="multipart/form-data">';
					html += '<input type="text" style="display: none;" name="rootFolderName" id="rootFolderName" value="'+para.rootFolderName+'">';
					html += '<input type="text" style="display: none;" name="relatedId" id="relatedId" value="'+para.relatedId+'">';
	            	...
	            	html += '					<input  id="fileImage" type="file" name="file" >';
	            	html += '					</div>';
	            	html += '				</div>';
				    ...
				    html += '							<img class="upload_image" src="../images/uploadFile/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>';
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				// 处理不同类型文件代表的图标
				var fileImgSrc = "../images/uploadFile/fileType/";
			...
			this.createCorePlug = function(){
				var params = {
					...
					rootFolderName: $("#rootFolderName").val(),   		
					...
					onSelect: function(selectFiles, allFiles) {
						if(para.multiple == false && FILE.uploadFile.length >= 1){
							$(".add_upload").hide();
							$(".webuploader_pick").hide();
							$(".upload_choose").hide();
						}
						para.onSelect(selectFiles, allFiles);  // 回调方法
						self.funSetStatusInfo(FILE.funReturnNeedFiles());  // 显示统计信息
						var html = '', i = 0;
						// 组织预览html
						...
						var funBindDelEvent = function(){
							if($(".file_del").length>0){
								...
								$(".file_del").click(function() {
									FILE.funDeleteFile(parseInt($(this).attr("data-index")), true);
									return false;	
								});
							}
							...
							if($(".file_edit").length>0){
								...
								$(".file_edit").click(function() {
									// 调用编辑操作
									//FILE.funEditFile(parseInt($(this).attr("data-index")), true);
									return false;	
								});
							}
						};
					...
					onSuccess: function(file, response) {
						...
						$.each(file,function(index,val){
							$("#uploadProgress_" + val.index).hide();
							$("#uploadSuccess_" + val.index).show();
							//$("#uploadInf").append("<p>上传成功，文件地址是：" + response + "</p>");
							// 根据配置参数确定隐不隐藏上传成功的文件
							if(para.finishDel){
								// 移除效果
								$("#uploadList_" + val.index).fadeOut();
								// 重新设置统计栏信息
								self.funSetStatusInfo(FILE.funReturnNeedFiles());
							}
							$("#uploadList_" + val.index+" .file_del").css("display","none");
						})
					},
			...
			this.addEvent = function(){
				...
				$(".upload_btn").bind("click", function(e){
					// 判断当前是否有文件需要上传
					if(FILE.funReturnNeedFiles().length > 0){
						$("#fileSubmit").click();
					}else{
						alert("请先选中文件再点击上传");
					}
