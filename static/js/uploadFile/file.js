var FILE = {
		...
		funUploadFiles : function(){
			var self = this;  // 在each中this指向没个v  所以先将this保留
			// 遍历所有文件  ，在调用单个文件上传的方法
			//self.funUploadFile(this.uploadFile,self.type,self.relatedId);
			...
			formdata.append("rootFolderName", self.rootFolderName);	         		
			formdata.append("relatedId", self.relatedId);	  
		    ...
		    xhr.upload.addEventListener("progress",	 function(e){
		    	// 回调到外部
		    	$.each(self.uploadFile, function(index, val){
		    		self.onProgress(val, e.loaded, e.total);
		    	});
		    }, false); 
		    // 完成
		    xhr.addEventListener("load", function(e){
		    	...
		    	if(response.success){
		    		var finshUploadFile = self.uploadFile;
		    		// 从文件中删除上传成功的文件  false是不执行onDelete回调方法
			    	$.each(self.uploadFile, function(index, val){
			    		self.funDeleteFile(val.index, false);
			    	});
			    	// 回调到外部
			    	self.onSuccess(finshUploadFile, response);
			    	if(self.uploadFile.length==0){
			    		// 回调全部完成方法
			    		self.onComplete("全部完成");
			    	}
		    	}else{
		    		self.onFailure(self.uploadFile, response);
		    	}
		    }, false);  
		    // 错误
		    xhr.addEventListener("error", function(e){
		    	// 回调到外部
		    	self.onFailure(self.uploadFile,JSON.parse(xhr.responseText));
		    }, false); 
			xhr.open("POST",self.url, true);
			xhr.send(formdata);
		},
		// 上传单个个文件
		funUploadFile : function(file,rootFolderName,relatedId){
			var self = this;  // 在each中this指向没个v  所以先将this保留
			var formdata = new FormData();
		    ...
		    xhr.addEventListener("load", function(e){
		    	...
		    	// 回调到外部
		    	self.onSuccess(file, JSON.parse(xhr.responseText));
		    	if(self.uploadFile.length==0){
		    		// 回调全部完成方法
		    		self.onComplete("全部完成");
		    	}
		    ...
		    xhr.addEventListener("error", function(e){
		    	// 回调到外部
		    	self.onFailure(file, JSON.parse(xhr.responseText));
		    }, false);  
