...
ace.handle_side_menu = function(a) {
	...
	a("#sidebar-collapse").on(ace.click_event, function() {
		...
		a("#main-content").toggleClass("main-content-large");
		ace.settings.sidebar_collapsed(!c)
		if($('.sidebar-shortcuts .sidebar-shortcuts-large').length==0){
			var html='<div class="sidebar-shortcuts-large gncdstyle" id="sidebar-shortcuts-large">'
				html+='<img src="'+path+'/images/skin_/menu.png">'
				html+='<span id="gncd">功能菜单</span>'
				html+='</div>';
				html+='<div class="sidebar-shortcuts-mini gncdstyle" id="sidebar-shortcuts-mini">';
				html+='<img src="'+path+'/images/skin_/menu.png">'
				html+='</div>';
			$('.sidebar-shortcuts').html(html);
		}
		if($('.sidebar-shortcuts .sidebar-shortcuts-mini').length==0){
			var html='<div class="sidebar-shortcuts-large gncdstyle" id="sidebar-shortcuts-large">'
				html+='<img src="'+path+'/images/skin_/menu.png">'
				html+='<span id="gncd">功能菜单</span>'
				html+='</div>';
		...
		if($("#sidebar").hasClass("menu-min")) {
			$(".sidebar-shortcuts .sidebar-shortcuts-mini").show();
			$(".sidebar-shortcuts .sidebar-shortcuts-large").hide();
		}else {
			$(".sidebar-shortcuts .sidebar-shortcuts-mini").hide();
			$(".sidebar-shortcuts .sidebar-shortcuts-large").show();
		}
	});
	...
	a(".nav-list").on(//左边导航条
			...
			function(h) {
				// 获取元素
				var obj = h.target || h.srcElement;
				if(typeof($(obj).attr("href")) == "undefined" ){
					obj = $(obj).closest("a");
				}
				if(typeof($(obj).attr("href")) != "undefined" && $(obj).attr("href") != "javascript:void(0)"&& $(obj).attr("href").length > 1){
					var main = 'admin/main?';
					if($(obj).attr("href").indexOf(main) > -1){
						$('#menuUrl').val("../"+$(obj).attr("hreflang"));
						$('#menuName').val($(obj).text());
					}else{//只要当前有地址都做请求
						$("#iframe").attr("src", $(obj).attr("href"));
					}
				}
				if(typeof($(obj).attr("href")) != "undefined" && $(obj).attr("href") != "javascript:void(0)"&& $(obj).attr("href").length > 1 && $(obj).attr("href").indexOf("#") < 0 ){
					$('#breadcrumbs_Current_page').html($(obj).text());
				}
					...
					d.find("> .open > .submenu").each(
							function() {
								if (this != f
										...
										"active")) {
									a(this).slideUp(500).parent().removeClass(
									"open")
								}
							})
					return
				...
				//展开(前提根菜单有地址就不能有子菜单，有子菜单则根菜单就不能有地址)
				if(typeof(obj.href) == "undefined" || obj.href == "javascript:void(0)" || obj.href.length < 1){
